...
import { SingleAuthors } from "../../hocks/authorsDetais";
import { SingUserPublications } from "../../hocks/singleUserPublication.hock";
const Authors = () => {
    console.log()
    const { name } = useParams();
    const user = SingleAuthors(name)
            ...
            <div className="w-full md:w-1/2 p-4">
                <div className="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div className="mb-4">
                        ...
                        <p className="text-gray-600">
                            <strong>Registration:  {user.length > 0 && user[0].registration}</strong>
                        </p>
                    </div>
                    ...
                    <ul className="list-disc list-inside">
                        {authors.map((publication, index) => (
                            <li key={index} className="text-gray-600 mb-2">
                                {publication.authors} {`"${publication.title}"`} {publication.date}
                            </li>
                        ))}
                    </ul>
                </div>
